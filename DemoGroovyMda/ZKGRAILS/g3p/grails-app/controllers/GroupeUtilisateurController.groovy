...
class GroupeUtilisateurController {
	...
	def list = {
		...
		[authorityList: GroupeUtilisateur.list(params)]
	}
	 ...
	 */
	def show = {
		def authority = GroupeUtilisateur.get(params.id)
		if (!authority) {
			flash.message = "GroupeUtilisateur not found with id $params.id"
	...
	def delete = {
		...
		flash.message = "GroupeUtilisateur $params.id deleted."
		redirect action: list
	}
	...
	def update = {
		...
		if (authority.version > version) {
			authority.errors.rejectValue 'version', 'authority.optimistic.locking.failure',
				'Another user has updated this GroupeUtilisateur while you were editing.'
	 ...
	 */
	def create = {
		[authority: new GroupeUtilisateur()]
	}
	...
	def save = {
		...
		def authority = new GroupeUtilisateur()
