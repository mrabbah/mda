...
/**
 * Risque Window Object
 **/
class RisqueWindow extends Window{
	/**
	* Logger de la class RisqueWindow
	**/
	private Log logger = LogFactory.getLog(RisqueWindow.class)
	...
	**/
	def select() {                    
        risque = risqueSelected	
		afficherValeurAssociation()
        //risque.lock()  //Ne peut etre utilisé que pour le base de donnée qui accepte le veruillage des enregisterments
        rafraichirField()
        activerBoutons(true)
    }
	/**
	* Fonction qui se charge de sauveguarder un nouveau élément de risque
	**/
    def add() {
	...
	**/
	...
	/**
	* Fonction qui se charge de mettre à jour un élément selectionné de risque
	**/
    def update() {
		actualiserValeurAssociation()
        risque.validate()
	...
	/**
	* Fonction qui se charge de supprimer un élément selectinné de risque
	**/
    def delete() {
        risque.delete(flush:true)
        activerBoutons(false)
        risque = new Risque()
        rafraichirField()
        rafraichirList()
	...
	/**
	* Permet d'afficher l'anglet d'ajout d'un nouveau risque
	**/
    def newRecord(){
	...
	**/
	...
	/**
	* Rafrichier la liste des risque
	**/
    def rafraichirList() {
        risques = Risque.list()
        def binder = new AnnotateDataBinder(this.getFellow("lstRisque"))
        binder.loadAll()		
	...
	**/
    def annulerSelection() {
        this.getFellow("lstRisque").clearSelection()
        risqueSelected = null
    }
	...
	**/
	def afficherValeurAssociation() {
		...
		def binderprojet = new AnnotateDataBinder(this.getFellow("coprojets"))
		projetSelected = projets.find{ it.id == Risque.findById(risque.id).projet.id }
        binderprojet.loadAll()
