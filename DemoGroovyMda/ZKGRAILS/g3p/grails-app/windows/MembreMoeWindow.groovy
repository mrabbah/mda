...
/**
 * MembreMoe Window Object
 **/
class MembreMoeWindow extends Window{
	/**
	* Logger de la class MembreMoeWindow
	**/
	private Log logger = LogFactory.getLog(MembreMoeWindow.class)
	...
	**/
	def membreMoes
	/**
	* membreMoe selectionné
	**/
	def membreMoeSelected
	/**
	* un nouveau element de membreMoe  
	**/
	def membreMoe
	...
	* Constructeur
	**/
	public MembreMoeWindow () {
        membreMoes = MembreMoe.list()
        membreMoeSelected = null
        membreMoe = new MembreMoe()
	...
	**/
	def select() {                    
        membreMoe = membreMoeSelected	
		afficherValeurAssociation()
        //membreMoe.lock()  //Ne peut etre utilisé que pour le base de donnée qui accepte le veruillage des enregisterments
        rafraichirField()
        activerBoutons(true)
    }
	/**
	* Fonction qui se charge de sauveguarder un nouveau élément de membreMoe
	**/
    def add() {
		actualiserValeurAssociation()
        membreMoe.validate()        
        if(!membreMoe.hasErrors()) {			
			try {
				membreMoe.save(flush:true)
			} catch(Exception ex) {
	...
	**/
	...
	/**
	* Fonction qui se charge de mettre à jour un élément selectionné de membreMoe
	**/
    def update() {
		actualiserValeurAssociation()
        membreMoe.validate()
	...
	/**
	* Fonction qui se charge de supprimer un élément selectinné de membreMoe
	**/
    def delete() {
        membreMoe.delete(flush:true)
        activerBoutons(false)
        membreMoe = new MembreMoe()
        rafraichirField()
        rafraichirList()
	...
	/**
	* Permet d'afficher l'anglet d'ajout d'un nouveau membreMoe
	**/
    def newRecord(){
	...
	**/
	...
	/**
	* Rafrichier la liste des membreMoe
	**/
    def rafraichirList() {
        membreMoes = MembreMoe.list()
        def binder = new AnnotateDataBinder(this.getFellow("lstMembreMoe"))
        binder.loadAll()		
	...
	**/
    def annulerSelection() {
        this.getFellow("lstMembreMoe").clearSelection()
        membreMoeSelected = null
    }
	...
	**/
	def actualiserValeurAssociation() {
		...
		membreMoe.projet = projetSelected
		if(projets.size() > 0) {
	...
	**/
	def afficherValeurAssociation() {
		...
		def binderprojet = new AnnotateDataBinder(this.getFellow("coprojets"))
		projetSelected = projets.find{ it.id == MembreMoe.findById(membreMoe.id).projet.id }
        binderprojet.loadAll()
