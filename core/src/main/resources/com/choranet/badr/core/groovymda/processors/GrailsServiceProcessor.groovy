...
class GrailsServiceProcessor extends GroovyModelProcessor implements IProcessing {
    ...
    void process(Map context) {
        ...
        getAllClasses(context.model).each { modelElement ->
            ...
            if (!fullyQualifiedName.startsWith("java") && fullyQualifiedName.size() > 0) {
                ...
                def templateName = "com/choranet/badr/core/groovymda/templates/GrailsService.gtl"

                // SET THE OUTPUT FILE NAME FOR THE FULLY QUALIFIED NAME
                def outputName = "grails-app/services/${fullyQualifiedName.replace('.','/')}Service.groovy"
        ...
        getAllClasses(context.model).each { modelElement ->
            ...
            if (!fullyQualifiedName.startsWith("java") && fullyQualifiedName.size() > 0) {
                ...
                // SET THE TEMPLATE TO USE
                def templateName = "com/choranet/badr/core/groovymda/templates/GrailsService_Unit_Tests.gtl"

                // SET THE OUTPUT FILE NAME FOR THE FULLY QUALIFIED NAME
                def outputName = "test/unit/${fullyQualifiedName.replace('.','/')}ServiceTests.groovy"
